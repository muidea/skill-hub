...
`serve` 写操作采用可选 `secretKey`：未配置 `--secret-key` 时，`GET` 类读取接口和 Web UI 仍可访问，但 `POST` / `DELETE` 等修改类 API 返回 `READ_ONLY`；配置后修改类 API 必须携带 `X-Skill-Hub-Secret-Key`。Web UI 管理端会在写操作需要时提示输入并暂存在当前浏览器会话；CLI bridge 可通过 `SKILL_HUB_SERVICE_SECRET_KEY` 传递该值。
