...
> 文档状态说明：
> 本文档主要保留为多仓库能力的历史设计背景与演进记录，不再作为当前实现边界的唯一依据。
...
> - `create` / `remove` / `validate` 只针对项目本地工作区，不服务化
...
- **无数据迁移**：本节为历史设计前提，当前实现不再强调兼容早期单仓库结构
- **保持现有工作流**：不改变项目工作空间管理（`.agents/skills/`）
- **冲突解决**：本文中的部分“用户选择机制”描述仅保留为历史讨论记录，当前归档策略以默认仓库统一归档为准
...
- 多仓库功能默认启用
- 反馈命令自动使用默认仓库作为归档目标
- `prune` 为独立的本地状态维护命令，用于清理 `state.json` 中因项目目录移动、删除而残留的失效项目记录，不参与多仓库同步
