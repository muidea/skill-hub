...
`v0.8.1` is a follow-up release for `v0.8.0` global skill management. It closes the lint issue found after the initial `v0.8.0` tag and makes release notes durable by letting both the local release script and GitHub Release workflow prefer tracked release note documents.
