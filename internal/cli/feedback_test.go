...
func TestUpdateRegistryVersion(t *testing.T) {
	tests := []struct {
		name        string
	...
	}{
		{
			...
			setup: func(t *testing.T) (string, string, func()) {
				skillHubHome, repoDir, _ := testutils.SetupTestSkillHub(t)
...
This is an updated test skill.`
				...
				if err := os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(skillContent), 0644); err != nil {
					t.Fatalf("更新技能文件失败: %v", err)
				}

				return skillHubHome, skillID, func() {
...
This is a new test skill.`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := updateRegistryVersion(skillID)
			if (err != nil) != tt.wantErr {
				t.Errorf("updateRegistryVersion() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if tt.wantErr && tt.errContains != "" && err != nil {
				if errStr := err.Error(); !contains(errStr, tt.errContains) {
					t.Errorf("updateRegistryVersion() error = %v, should contain %v", errStr, tt.errContains)
				}
			}
